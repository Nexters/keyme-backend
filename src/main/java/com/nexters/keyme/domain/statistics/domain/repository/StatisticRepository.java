...
public interface StatisticRepository extends JpaRepository<Statistic, Long> {
    ...
    @Query(value = "SELECT * FROM statistic st WHERE (st.solver_avg_score < :cursorScore OR st.solver_avg_score = :cursorScore AND st.id > :cursor) AND st.id NOT IN :exceptIds ORDER BY st.solver_avg_score DESC, st.id LIMIT :limit", nativeQuery = true)
